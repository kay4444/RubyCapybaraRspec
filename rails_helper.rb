...
RSpec.configure do |config|
  ...
  if ["chrome", "firefox", "safari", "opera"].include? environment
    environment == 'chrome' ? (Selenium::WebDriver::Chrome.driver_path = (OS.windows? ? "#{ROOT_DIRECTIRY}/Resources/chromedriver.exe" : "#{ROOT_DIRECTIRY}/Resources/chromedriver")) : "Cromedriver need to be set just for Chrome browser."
    environment == 'firefox' ? (Selenium::WebDriver::Firefox.driver_path = (OS.windows? ? "#{ROOT_DIRECTIRY}/Resources/geckodriver.exe" : "#{ROOT_DIRECTIRY}/Resources/geckodriver")) : "Geckodriver need to be set just for Firefox browser."
  ...
  config.after(:each) do |example_group|
    Capybara.save_path = "#{ROOT_DIRECTIRY}/Reports/" # path where screenshots are saved
